...
from utils.loss_utils import calculate_loss, l1_loss, get_img_grad_weight, lncc, smooth_loss
# from gaussian_renderer import render_surfel, render_initial, render_volume, network_gui
from gaussian_renderer import render_initial,render_volume
...
def load_normal_prior(dataset, scan_id:str, r=1, method="GeoWizard", args=None):
    ...
    mask_dir =  get_mask_dir(dataset.source_path)
    print(mask_dir)
    ...
    for mpath in mask_paths:
        ...
        img_name = os.path.basename(mpath)[:-4]
        mask_images.update(
...
def mono_normal_loss(viewpoint_cam, surf_normal, rend_normal, mask_images, normal_images, gamma, iteration, ref_mask=None, HSV_mask=None):
    ...
    def get_normal_loss(normal_pred, normal_gt, mask=None):       
        ...
        if mask is None:
            ...
            cos = ((1. - torch.sum(normal_pred * normal_gt, dim=-1))*mask.squeeze()).mean()
            return l1, cos
...
def calc_warp_loss(
    ...
    original_weight = ref_weight = pixels = None
    # albeldo,mtl,rgh = albeldo_images[image_name].cuda(),mtl_images[image_name].cuda(),rgh_images[image_name].cuda() 
    # (3,H,W)
    geo_loss = ncc_loss = base_color_loss = base_color_loss_map = None
    metallic_warp_loss = roughness_warp_loss = None
    ...
    H,W=render_pkg['surf_depth'].squeeze().shape
    return geo_loss, ncc_loss, base_color_loss, metallic_warp_loss, roughness_warp_loss ,original_weight, visual_refweight(ref_weight,pixels,H,W),None# base_color_loss_map
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, model_path, args, debug_from=None):
    ...
    while iteration < TOT_ITER:
        ...
        if not initial_stage and iteration > 10000 and viewpoint_cam.image_name in ref_score_images.keys():
            ...
            ref_score_loss_metallic = torch.abs(refl_strenghth_map - .9)[ref_score_image]
            ref_score_loss_roughness = torch.abs(roughness_map-0.05)[ref_score_image]
            # ref_score_loss_metallic = torch.exp(ref_score_loss_metallic)
            # ref_score_loss_roughness = torch.exp(ref_score_loss_roughness)
            ref_score_loss_metallic = ref_score_loss_metallic.mean()
            ref_score_loss_roughness = ref_score_loss_roughness.mean()
            total_loss += opt.ref_score_loss_weight * ref_score_loss_metallic
            total_loss += opt.ref_score_loss_weight * ref_score_loss_roughness
            
            ref_score_loss_metallic2 = torch.abs(refl_strenghth_map-0.05)[~ref_score_image]
            ref_score_loss_roughness2 = torch.abs(.9 - roughness_map)[~ref_score_image]
            # ref_score_loss_metallic2 = torch.exp(ref_score_loss_metallic2)
            # ref_score_loss_roughness2 = torch.exp(ref_score_loss_roughness2)
            ref_score_loss_metallic2 = ref_score_loss_metallic2.mean()
            ref_score_loss_roughness2 = ref_score_loss_roughness2.mean()
            total_loss += opt.ref_score_loss_weight * ref_score_loss_metallic2
            total_loss += .5 * opt.ref_score_loss_weight * ref_score_loss_roughness2

            # albeldo smooth
            total_loss += smooth_loss(render_pkg["base_color_map"] * ref_score_image)
            # ref_score_image = ref_score_images[viewpoint_cam.image_name]
            # refl_strenghth_map = render_pkg["refl_strength_map"]
            # roughness_map = render_pkg["roughness_map"]
        ...
        # ##### start calc reflection score
        # def valid_iters(iters): 
        #     return ((iters % 3000) >= 500) and (iters % 200 == 0)

        # if iteration > 12000 and valid_iters(iteration):
        #     print("Calculating Reflection Score...")
        #     ref_score_images = calc_ref_score(
        #         scene, opt, gaussians, dataset, pipe,
        #         albedo_images, mtl_images, rgh_images, mask_images,
        #         iteration, background
        #     )
        # ##### end calc reflection score
        ...
        with torch.no_grad():
            ...
            if iteration % VISUAL_INTERVAL == 0 or iteration == first_iter + 1 or iteration == opt.volume_render_until_iter + 1:
                save_training_vis(viewpoint_cam, gaussians, background, render, pipe, opt, iteration, initial_stage, d_mask, ref_weight, base_color_loss_map,render_pkg,None) #ref_score_images[viewpoint_cam.image_name])
...
def evaluate_psnr(scene, renderFunc, renderkwargs):
    ...
    if len(scene.getTestCameras()):
        for viewpoint in scene.getTestCameras():
            ...
            image = torch.clamp(render_pkg["render"], 0.0, 1.0)
            try:
                save_image(image, "./outputs/evaluate/"+viewpoint.image_name+".png")
            except:
                pass
            gt_image = torch.clamp(viewpoint.original_image.to("cuda"), 0.0, 1.0)
            psnr_test += psnr(image, gt_image).mean().double()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_iterations", nargs="+", type=int, default=[10000,14000,15000,16000,20000,30000,40000,50000,60000])
    parser.add_argument("--quiet", action="store_true")
    parser.add_argument("--checkpoint_iterations", nargs="+", type=int, default=[10000,14000,12000,15000,16000,20000,30000,40000,50000,60000])
    parser.add_argument("--start_checkpoint", type=str, default = None)
    # Prior paths
    parser.add_argument("--geowizard_path", type=str, default=f"/{os.getcwd().split('/')[1]}/tjm/code/GeoWizard/output")
    parser.add_argument("--metric3d_path", type=str, default=f"/{os.getcwd().split('/')[1]}/tjm/code/Metric3D/output")
    parser.add_argument("--idarb_path", type=str, default=f"/{os.getcwd().split('/')[1]}/tjm/code/IDArb/output")
    parser.add_argument("--ref_score_path", type=str, default=f"/{os.getcwd().split('/')[1]}/tjm/code/ref-gaussian-tjm/tmp_ref_scores_merge_mask2")
