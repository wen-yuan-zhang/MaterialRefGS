...
from gaussian_renderer import GaussianModel,depth_to_normal,eval_sh
...
class HardwareRendering(nn.Module):
    ...
    def build_bvh(self, pcd: GaussianModel, rebuild: int = 1):
    ...
    def render_gaussians(self,
        ...
        v, f = self.build_bvh(pcd, rebuild=self.training)
        ...
        if True:#pcd.get_specular.shape[0] != means3D.shape[0]:
            others_precomp = torch.cat([
                torch.ones_like(opacities) * 0.01, torch.ones_like(opacities) * 0.01], 
            dim=-1).contiguous()  # (P, 2)
        # For reflection rendering, we need to render specular and (roughness, no use for now)
        else:
            others_precomp = torch.cat([pcd.get_specular, torch.ones_like(opacities) * pcd.init_roughness_value], dim=-1).contiguous()  # (P, 2)
        ...
        # Prepare output dict
        output = {
            "viewspace_points":grads3D,  # (P, 3) # for densify
        ...
        if start_from_first:
            ...
            surf_norm = depth_to_normal(camera, dpt.permute(2, 0, 1))  # (H, W, 3)
            # Remember to multiply with accum_alpha since render_normal is unnormalized
            surf_norm = surf_norm * acc.detach()  # (H, W, 3)
            # Update the output
            output.update({"surf_normal":surf_norm.permute(2, 0, 1)}) # (3,H,W)
        else:
            output.update({"surf_normal":torch.zeros_like(norm).permute(2, 0, 1)})

        # Return the reflection properties if needed
        specular_channels = 1
        output.update({
            "specular":aux[..., :specular_channels].permute(2, 0, 1),  # (1, H, W)
            "roughness":aux[..., specular_channels:specular_channels+1].permute(2, 0, 1)  # (1, H, W)
        })
