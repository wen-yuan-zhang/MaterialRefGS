...
class Scene:
    ...
    def __init__(self, args : ModelParams, gaussians : GaussianModel, load_iteration=None, shuffle=True, resolution_scales=[1.0], env_gaussians=None):
        """b
        ...
        self.env_gaussians = env_gaussians
        ...
        if self.loaded_iter:
            ...
            env_path = os.path.join(self.model_path,
                                                        "point_cloud",
                                                        "iteration_" + str(self.loaded_iter),
                                                        "env_point_cloud.ply")
            if os.path.exists(env_path) and self.env_gaussians is not None:
                env_gaussians.load_ply(env_path)
        else:
    ...
    def save(self, iteration):
        ...
        if self.env_gaussians is not None and self.env_gaussians.get_xyz.shape[0] > 0:
            self.env_gaussians.save_ply(os.path.join(point_cloud_path, "env_point_cloud.ply"))

    def get_env_gaussians(self):
        return self.env_gaussians
