...
class EnvGaussianModel:
    ...
    def freeze_by_name(self, anchored_lst=[]):
        from collections import defaultdict
        lr_dict = defaultdict(lambda: 0.0)
        for param_group in self.optimizer.param_groups:
            if param_group["name"] in anchored_lst:
                lr_dict[param_group["name"]] = param_group['lr']
                param_group['lr'] = 0.0
        self.lr_dict2 =  lr_dict
        self.anchored_lst2 = anchored_lst
    
    def unfreeze_by_name(self):
        for param_group in self.optimizer.param_groups:
            if param_group["name"] in self.anchored_lst2:
                param_group['lr'] = self.lr_dict2[param_group["name"]]
    ...
    def freeze_geo(self):
        ...
        for param_group in self.optimizer.param_groups:
            if param_group["name"] == "xyz":
                lr_dict["xyz"] = param_group['lr']
                param_group['lr'] = 0.0
            if param_group["name"] == "ratation":
                lr_dict["ratation"] = param_group['lr']
                param_group['lr'] = 0.0
            if param_group["name"] == "scaling":
                lr_dict["scaling"] = param_group['lr']
                param_group['lr'] = 0.0
            if param_group["name"] == "opacity":
                lr_dict["opacity"] = param_group['lr']
                param_group['lr'] = 0.0
    ...
    def unfreeze_geo(self):
        for param_group in self.optimizer.param_groups:
            ...
            if param_group["name"] == "ratation":
                param_group['lr'] = self.lr_dict["ratation"]
            if param_group["name"] == "scaling":
    ...
    @torch.no_grad()
    def update_env_gs_(self,iter,opt,scene,render_pkg):
        env_densify_untile_iter = 30000
        env_densify_inter = 500
        env_opacity_reset_inter = 6000
        env_densify_grad_thres = 1e-4
        
        visibility_filter = render_pkg["visibility_filter"]
        viewspace_point_tensor = render_pkg["viewspace_points"]
        # radii = render_pkg["radii"]
        if viewspace_point_tensor.grad is None: return
        if iter == 30000:
            self.freeze_geo()
            return 
        if iter > 30000:
        ...
        self.update_learning_rate(iter)
        if iter % 1000 == 0:
            self.oneupSHdegree()

        # if iter > 0 and iter < env_densify_untile_iter and render_pkg is not None:
        ...
        if iter <= env_densify_untile_iter and iter % env_densify_inter == 0:
            size_threshold = 20 if iter > env_opacity_reset_inter else None
            self.densify_and_prune(env_densify_grad_thres, opt.prune_opacity_threshold, scene.cameras_extent,size_threshold)
    ...
    def update_env_gs(self,iter,opt,scene,render_pkg):
        self.update_env_gs_(iter,opt,scene,render_pkg)
        return
        env_densify_until_iter = 18000
        env_densify_inter = 2000
        env_opacity_reset_inter = 6000
        env_densify_grad_thres = 1e-4
        env_densify_start_inter = 12510
        ...
        self.add_densification_stats(viewspace_point_tensor, visibility_filter)
        # return
        if iter <= env_densify_until_iter and iter % env_densify_inter == 0:
            print("Total points: ", self.get_xyz.shape[0], f"Scene extend: {scene.cameras_extent}" ,"Densify and prune before")
            size_threshold = 20 if iter > env_opacity_reset_inter else None
            self.densify_and_prune(env_densify_grad_thres, 0.05, scene.cameras_extent,size_threshold)
            print("Total points: ", self.get_xyz.shape[0], "Densify and prune done")

            if iter % env_opacity_reset_inter == 0 or iter == env_densify_start_inter:
                self.reset_opacity()
