...
def save_transformer_matrix(cam_infos):
    import json
    ...
    for cam_info in cam_infos:
        ...
        frames.append(
            {
                "file_path": cam_info.image_name,
                "transform_matrix": c2w.tolist()
            }
        )
               
    fp["frames"] = frames
    with open("transforms/transformers.json","w") as f:
        f.write(json.dumps(fp,indent=1))
...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder):
    ...
    save_transformer_matrix(cam_infos)
    return cam_infos
