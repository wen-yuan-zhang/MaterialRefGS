...
class RayTracer():
    def __init__(self, vertices, triangles, ply_path=None):
        ...
        if ply_path is not None:
            ...
            # self.vertex_colors = nn.Parameter(self.vertex_colors)
            # for k_,v_ in self.vertex_attrs.items():
            #     if k_ in ["diffuse","roughness","metallic","albedo"]:
            #         self.vertex_attrs[k_] = nn.Parameter(v_)
    ...
    def secondary_indirect_color(self, surface_pos:torch.Tensor, rays_v:torch.Tensor,face_normals:torch.Tensor, triangle_indices: torch.Tensor, hit_depth:torch.Tensor, env_map:EnvLight):
        ...
        vertex_albedo = self.vertex_attrs["albedo"][vertex_ids].reshape((-1,3,3)).float()
        ...
        diffuse = self.barycentric_interpolation(surface_pos_inv,vertex_pos,vertex_diffuse).float() #(N,3)
        refl_strength = self.barycentric_interpolation(surface_pos_inv,vertex_pos,vertex_metallic) #(N,1)
        roughness = self.barycentric_interpolation(surface_pos_inv,vertex_pos,vertex_roughness) #(N,1)
        albedo = self.barycentric_interpolation(surface_pos_inv,vertex_pos,vertex_albedo) #(N,3)
        normal_map = self.barycentric_interpolation(surface_pos_inv,vertex_pos,vertex_normal)
